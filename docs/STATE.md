...
- **Current runner (2026-09-24): GitHub-hosted.** `AUTOMATION_RUNNER` is unset because arXiv's CDN has been sending the Mac Mini a 406 on every request since 2026-09-18. The Mac Mini still triggers the digest. Move back with `scripts/runner.sh local` once MS-007 is resolved (D-037)
...
- In-process: exponential backoff with jitter (~30s, 60s, 120s, 240s, 480s cap), 7 attempts; honors Retry-After. Retryable set: any 5xx, 406, 408, 429, plus ConnectError/ReadTimeout. A 406 (CDN block) gets only 3 attempts, so a persistent block reaches the workflow retry fast (D-037)
...
- Workflow timeout 150 min to accommodate three attempts. Known gap: a persistent 429/5xx can still overrun it, and the run ends *cancelled* (no failure email) rather than *failed* (D-037)
