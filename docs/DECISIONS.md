...
## D-029
- Date: 2026-05-20
- Status: Superseded by D-035 (chain refreshed; the 2.5 models this entry deliberately retained were dropped)
...
## D-030
- Date: 2026-05-20
- Status: Accepted; scheduling partially superseded by D-036 (the weekly GitHub cron created here is now dispatched by digest.yml; the CLI and the exit-non-zero-on-drift design stand)
...
## D-031
- Date: 2026-06-01
- Status: Accepted; amended by D-037 (406 gets a short in-process retry budget)
...
- Date: 2026-06-01
- Status: Accepted
- Decision: On a **self-hosted macOS** runner, skip `actions/setup-python` and build a venv from the runner's pre-installed Python 3.12 (`python3.12 -m venv "$RUNNER_TEMP/venv"`, prepended via `$GITHUB_PATH`) in both `digest.yml` and `check-models.yml`. The setup-python step is gated `if: !(vars.AUTOMATION_RUNNER != '' && runner.os == 'macOS')` and the venv step `if: vars.AUTOMATION_RUNNER != '' && runner.os == 'macOS'`, so GitHub-hosted (any OS) and Linux self-hosted runners keep `actions/setup-python` unchanged.
- Rationale: `actions/setup-python`'s macOS path installs the python.org `.pkg` via `sudo installer -pkg ... -target /`, which needs passwordless sudo. GitHub-hosted macOS runs as `runner` (NOPASSWD); a typical self-hosted macOS box runs as a normal user without it, so the step fails (`mkdir: /Users/runner: Permission denied`, then the sudo install). An always-on self-hosted box already has a managed Python (pyenv/Homebrew), so a venv from it is simpler and sudo-free. Scoped to macOS-self-hosted (not all self-hosted) so Linux self-hosted forks — where setup-python works — are unaffected; the venv assumes Python 3.12 is on the runner user's PATH (documented in `docs/RUNNER.md`).

## D-034
- Date: 2026-06-01
- Status: Accepted; partially superseded by D-036 (check-models.yml no longer keeps its weekly GitHub cron)
- Decision: Trigger the daily digest via an **external scheduler** — an OS timer (the maintainer uses a macOS launchd LaunchAgent) running `gh workflow run digest.yml` (`workflow_dispatch`) — instead of GitHub's `schedule:` cron. Removed the `schedule:` trigger from `digest.yml` (left commented so forks can re-enable GitHub cron); `workflow_dispatch` remains. `check-models.yml` keeps its weekly GitHub cron (delay-tolerant). Partially supersedes D-032's "keep a single scheduler (GitHub cron) in charge" for the digest.
- Rationale: GitHub batches/delays `schedule` events (often many minutes to hours under load, trending worse), so the digest arrived late and unpredictably — defeating a "morning" digest. `workflow_dispatch` dispatches within seconds, so an external timer fires it on time; it still executes on the self-hosted runner (D-032/D-033). D-032 had rejected a local scheduler because it "would need GitHub's cron disabled and risks double-sends" — both are resolved by *removing* the `schedule:` trigger entirely, so exactly one scheduler (the external timer) is in charge and there is no double-send. The owner already runs an always-on self-hosted runner (D-032), so a co-located timer adds no new always-on dependency. Trade-offs: a fork gets no automatic run out of the box (mitigated by the commented-out `schedule:` block + `docs/RUNNER.md`); and the timer, being a per-user agent, only fires while the user's login session is active, so a reboot with no login misses that day until next login.

## D-035
- Date: 2026-07-27 (chain listing refreshed 2026-09-03)
- Status: Accepted
- Decision: Keep a five-model Gemini fallback chain: two Flash + two Lite + Gemma, best-quality-first, all free-tier and `generateContent`-capable. Current order (2026-09-03): `gemini-3.8-flash → gemini-3.7-flash → gemini-3.5-flash-lite → gemini-3.1-flash-lite → gemma-4-31b-it` (added 3.8/3.7 Flash, dropped 3.6/3.5 Flash). Original 2026-07-27 order was `gemini-3.6-flash → gemini-3.5-flash → gemini-3.5-flash-lite → gemini-3.1-flash-lite → gemma-4-31b-it`; that refresh dropped `gemini-3-flash-preview` and both 2.5 models. In the same original change: drop the deprecated `temperature` param from `generationConfig`, move the API key from the `?key=` query param to an `x-goog-api-key` client header (matching `model_check.py`), replace the partial-key init log with a SHA-256 fingerprint, and log a summary line when summaries fall back to extractive. Supersedes the chain in D-029.
- Rationale: Google shipped `gemini-3.6-flash` and `gemini-3.5-flash-lite` GA on 2026-07-21 and deprecated `temperature`/`top_p`/`top_k` in the same release; the drift checker (D-030) caught the models but structurally cannot catch the param change, since it diffs model IDs only. The 2.5 models shut down 2026-10-16, so the old chain's effective depth would have dropped from 6 to 4 in October, whereas this one holds until 2027-05-07 (`gemini-3.1-flash-lite`, the only entry with an announced shutdown). Production logs justify the ordering: run 30091502860 fell through three positions on 2026-07-24 with only 503s and timeouts and zero 429s, and what recovered it was a *lite* model on a different serving tier. So the chain buys availability, not quota. That inverts the original quota rationale for `gemma-4-31b-it` but replaces it with a stronger one, since Gemma is the only non-Gemini family and therefore the only entry decorrelated from a flash-wide outage; removing it would make the chain fully correlated with the exact failure seen weekly. Digests summarize 5-7 papers, far below any plausible rate limit, and Google no longer publishes per-model free-tier RPD. Verified before shipping that all five models are present and `generateContent`-capable, that header auth returns 200 for `:generateContent`, and that `gemini-3.6-flash` ignores `temperature` outright (0.0 still returns varying output over repeated samples), so the param removal is behaviourally a no-op rather than a quality change. The key move matters because httpx logs full request URLs at INFO and the digest runs `--verbose`, so the key was written to a log line on every call, unmasked outside Actions.

## D-036
- Date: 2026-07-27
- Status: Accepted
- Decision: Remove the `schedule:` cron from `check-models.yml` (left commented for forks) and dispatch it instead from a new Mondays-only `dispatch-model-check` job in `digest.yml`, which calls `gh workflow run check-models.yml` under a job-level `actions: write` grant. The job is independent of the `digest` job and pinned to a GitHub-hosted runner. Partially supersedes D-030, which created the cron, and D-034, which reaffirmed it.
- Rationale: The repo is public and GitHub auto-disables scheduled workflows after 60 days of repo inactivity, a rule already recorded in `docs/STATE.md` but never mitigated. With the digest moved to external dispatch by D-034, `check-models.yml` became the only scheduled workflow, and GitHub warned it would be disabled on ~2026-07-31, which would have stopped drift detection silently and precisely when a quiet repo most needs it. The cron was also firing 2-3.5 hours late every week (12:47 UTC scheduled; 15:30, 14:57, 15:34, 16:05 and 16:27 observed), so the delay-tolerance argument in D-034 was weaker than assumed. Dispatching from the digest reuses a trigger already proven to fire within seconds every day, so no new timer exists that could fail silently and nothing has to be installed on the self-hosted box. Considered and rejected: a second OS timer on the always-on machine (a new silent-failure path, and unverifiable from the maintainer's laptop), and keeping the cron alive by pushing at least once every 60 days (fragile, and depends on remembering). Trade-offs: `digest.yml` now knows about the drift check, and if the digest's external timer stops then drift detection stops with it. That is judged the safer coupling, because a missing digest is immediately obvious in the inbox whereas a silently disabled cron is not. The job needs the explicit `actions: write` permission because the repo default workflow permission is read-only and `digest.yml` declares none.

## D-037
- Date: 2026-09-24
- Status: Accepted; amended by D-038 (429 now shares the short budget; corrections below)
- Decision: (1) A 406 from arXiv gets at most `BLOCKED_MAX_RETRIES = 2` in-process retries (about 1.5 min) instead of the full `DEFAULT_MAX_RETRIES = 6` backoff (about 27 min). It stays retryable and still escalates to `ArxivTransientError` / exit 75, so the workflow's fresh-runner retry is unchanged. All other retryable statuses keep the full budget. (2) Scheduled automation temporarily runs on GitHub-hosted runners (`AUTOMATION_RUNNER` unset on 2026-09-24) while the Mac Mini's block is diagnosed (MS-007). Triggering is unchanged: the Mac Mini's timer still dispatches `digest.yml`.
- Rationale: From 2026-09-18 to 2026-09-24 every digest run on the self-hosted Mac Mini got an instant 406 (empty body, answered at the Fastly edge without reaching the Google Frontend origin) on all 21 requests. The workflow's own `curl -sI` diagnostic from the same box got a 200 seconds earlier, and the exact production request sent from another Mac with curl, httpx and urllib all got 200s. So the block is specific to that machine or its IP, not to the query, User-Agent, Accept header or HTTP client. A persistent block can't be cleared by backing off on the same IP, and the full budget x3 workflow attempts plus 90 min of sleeps exceeds the 150-min job timeout, so runs ended as *cancelled*, which sends no failure notification. Five digests were silently missed. Removing 406 from the retryable set was rejected: commit `1a590f9` (2026-04-16) added it because GitHub-hosted runners got transient 406s that retrying cleared, and the digest is on GitHub-hosted runners again. The short budget keeps that recovery while making a persistent block end as a *failed* job (about 1.5 hours) and cutting the requests sent into a block from 21 to 9 a day. Moving the runner does not bring back the D-034 scheduling delays, which came from GitHub's `schedule:` cron, not from GitHub-hosted runners. The first GitHub-hosted run on 2026-09-24 was picked up within seconds and got a 200 from arXiv on its first request.
- Known gap: 429/5xx still use the full budget, so a persistent 429/5xx can still overrun the timeout and end as cancelled. (429 closed by D-038; 5xx remains.)
- Corrections (2026-09-24, after the Mac Mini diagnosis in MS-007): (a) the "exact production request from another Mac" tests ran through Claude Code's sandbox proxy, which intercepts TLS, so arXiv saw the proxy's TLS handshake, not httpx's or curl's. They do not show the HTTP client is irrelevant. The stronger evidence is from the Mac Mini itself: an identical stock httpx GET got 406 at 13:55:55 EDT and 200 at 13:58:25 EDT with nothing changed on the machine. (b) The workflow's `curl -sI` diagnostic is not a health signal: on GitHub-hosted run 36021616143 it got a 406 while the httpx GET got a 200, the reverse of the Mac Mini. The block was keyed on the IP plus some request attribute and expired on its own after roughly a week.

## D-038
- Date: 2026-09-24
- Status: Accepted
- Decision: Treat 429 like 406: `THROTTLED_STATUSES = (406, 429)` get at most `THROTTLED_MAX_RETRIES = 2` in-process retries, then raise `ArxivTransientError` (exit 75) so the workflow's 30/60-min cool-down retry takes over. Renamed `BLOCKED_MAX_RETRIES` to `THROTTLED_MAX_RETRIES`. 5xx, 408 and network errors keep the full `DEFAULT_MAX_RETRIES = 6` budget. Stay on GitHub-hosted runners through at least the 2026-09-25 run before moving back to the Mac Mini.
