...
- Tests: reshaped the fake API-key fixture so it no longer looks like a real Google key. Written as `AIza...` for realism, it tripped GitHub secret scanning as a Google API Key. No assertion depended on the shape, so the fixture is now unmistakably not a credential
